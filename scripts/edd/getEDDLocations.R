...
getEDDLocations <- function(results_list, marc2022, marc2021, habitat_marc2021, habitat_marc2022, addMarc){
    tryCatch(
        expr = {
            ...
            ncrn_fish_locations <- getNCRNFishLocations(results_list, example) # updated to NCRN_Site_ID
            ...
            if(addMarc==TRUE){
                marc2022_locations <- getMarc2022Locations(marc2022, example, results_list) # updated to NCRN_Site_ID
                marc_habitat_locations <- getMarcHabLocations(habitat_marc2022, habitat_marc2021, example, results_list)# updated to NCRN_Site_ID # NCRN doesn't have any of Marc's habitat data, so add both 2021 and 2022
